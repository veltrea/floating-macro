...
public struct PanelConfig: Codable, Equatable {
    ...
    public var dockedEdge: DockEdge?        // 縁にドック (Phase 3.5 で Bool→DockEdge? に変更)
...
- [x] **P3.5-1** `EdgeDockBar` (新 NSPanel サブクラス) — 矩形バー描画、preset アイコン + 名前表示、左クリックで展開・右クリックでメニュー
- [x] **P3.5-2** 端検出ロジック — `EdgeDetector.nearestEdge` (Core 層の純粋関数)
- [ ] **P3.5-3** ドック化 / 展開のアニメーション（スコープ外: 初期は即切替）
- [x] **P3.5-4** メニューバーに「縁にドック ▸」サブメニュー追加、ドック中パネルに「展開」「別の辺に移動 ▸」
- [x] **P3.5-5** Control API: `panel_dock` `{id, edge?}` / `panel_undock` `{id}` ツール
- [x] **P3.5-6** Tests: `EdgeDetectorTests` (6 ケース)、`EdgeDockLayoutTests` (7 ケース)、`AppConfigPanelOpsTests` にドック/アンドック 3 ケース、`ConfigLoaderTests` に旧互換 3 ケース、`ToolCatalogTests` に Phase 3.5 ツール 3 ケース
