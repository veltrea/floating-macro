...
- [ ] **P2-1** `GroupDisplayType` enum 定義 + `ButtonGroup.displayType` 追加（後方互換 decoder、既定値 `.icon`）
- [ ] **P2-2** Wide タイプのレンダラー実装（横長、ラベル中心、アイコン左寄せ）
- [ ] **P2-3** Card タイプのレンダラー実装（サムネイル上 + タイトル下、アスペクト比可変）
- [ ] **P2-4** `ButtonDefinition.thumbnail` フィールド追加 + サムネ画像のロード
- [ ] **P2-5** サムネイル保存規約：`presets/<name>/images/<button-id>.{ext}`
- [ ] **P2-6** 設定画面でグループ表示タイプ切替セレクター
- [ ] **P2-7** Card タイプ用のサムネ追加 UI（ファイル選択 + ドロップ）
- [ ] **P2-8** F7: ボタン押下中の枠線色変化（黄→緑/赤→既定）
- [ ] **P2-9** F7: 各 Action の成功/失敗判定の整理（戻り値が無いものは「実行完了」を成功扱い）
- [ ] **P2-10** Tests: displayType 切替が JSON ラウンドトリップで保たれる
- [ ] **P2-11** Tests: 旧プリセット（displayType 無し）が `.icon` でロードされる
- [ ] **P2-12** SPEC.md 更新、サンプル preset 追加（card タイプの Midjourney 風サンプル）
