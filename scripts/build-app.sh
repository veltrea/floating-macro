...
# ------------------------------------------------------------------- #
# 0.5  Generate BuildInfo.generated.swift
# ------------------------------------------------------------------- #
# Every build gets a fresh file with commit hash, timestamp, and source
# directory. This makes it trivial to tell which build is running,
# especially when working across worktrees.

BUILDINFO="$ROOT/Sources/FloatingMacroApp/BuildInfo.generated.swift"
say "Generating BuildInfo …"

GIT_HASH="$(git -C "$ROOT" rev-parse --short HEAD 2>/dev/null || echo "unknown")"
GIT_DIRTY="$(git -C "$ROOT" diff --quiet 2>/dev/null && echo false || echo true)"
...
enum BuildInfo {
    ...
    static let sourceDir = "$SOURCE_DIR"
}
SWIFT
ok "BuildInfo: $GIT_HASH (dirty=$GIT_DIRTY) branch=$GIT_BRANCH"
