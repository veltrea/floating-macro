...
expect_http "preset reorder"                 200  POST /preset/reorder \
  '{"ids":["api-created","alt","default"]}'
# Verify /preset/list reflects the new order
order_out=$(curl -sS --max-time 3 "http://127.0.0.1:$PORT/preset/list")
first_name=$(printf '%s' "$order_out" | python3 -c "
import sys, json
d=json.load(sys.stdin)
print(d['presets'][0]['name'])
" 2>/dev/null)
if [ "$first_name" = "api-created" ]; then
    pass=$((pass+1)); say "✓  preset reorder reflected in /preset/list"
else
    fail=$((fail+1)); failures+=("preset reorder not reflected (first=$first_name)")
    say "✗  preset reorder not reflected (first=$first_name)"
fi
expect_http "preset reorder bad body"        400  POST /preset/reorder '{}'
