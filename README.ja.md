...
制御 API は本体起動時に自動で `127.0.0.1:17430` に bind されます。全エンドポイントが Bearer トークン認証必須（`/manifest` 等のディスカバリー系を除く）。トークンは初回起動時に Keychain に自動生成されるので、ユーザーが何かを設定する必要はありません。
...
トークンは `security find-generic-password -s FloatingMacro -a ControlAPIToken -w` で取得。
...
最新リリース: **v0.5** (2026-04-29)。変更履歴は [CHANGELOG.md](CHANGELOG.md)、ロードマップは [SPEC.md §17](SPEC.md) を参照してください。公開リリースですが、作者は安定性を保証しません。Pull Request や Issue は歓迎します。
