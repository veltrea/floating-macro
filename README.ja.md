...
- **パネル背景色のカスタマイズ** (v0.15) — プリセット単位でパネルの背景色を変更可能。右クリックメニュー「背景色 ▸」またはカラーピッカーで設定
- **ジニーエフェクト & ドックバー操作強化** (v0.15) — パネル→ドック遷移に SpriteKit メッシュワープによるジニーエフェクト。ドックバーはドラッグで自由配置、位置は再起動後も保持
- **縁にドック最小化** (v0.14) — パネルを画面端に薄いバーとして格納。左クリックで展開、右クリックでメニュー
- **スマホ / タブレットからの操作** (v0.13) — メニューバーまたは各フローティング右上の QR ボタンから「📱 デバイスに送信」。同じ Wi-Fi のスマホ Safari でカメラから読み取ると、その場でパネルが開きボタンを押せる。Stream Deck のソフトキー領域に対する OSS 代替
- **複数のフローティングパネル** (v0.12) — 用途別に独立したパネルを同時表示できる。各パネルが別のプリセットを表示、位置・サイズ・透明度を独立に持ち、メニューバーから追加/切替/削除
- **グループの表示タイプ icon / wide / card** (v0.11) — ボタンを「小型アイコン」「横長セル」「サムネイル付きカード」の 3 種類でレンダリングできる。MidJourney 等のプロンプトギャラリー用途には card + appendMode の組み合わせが最適
- **ボタンへのサムネイル画像** (v0.11) — card タイプのボタンに大判画像を割り当て可能。`presets/<name>/images/<button-id>.<ext>` に自動保存
- **6 種類のアクション** — key / text / launch / terminal / delay / macro
- **プロンプトビルダー (text の追記モード)** (v0.10) — text アクションに `appendMode` を追加。複数のボタンを連打すると現在のクリップボードに断片が連結され、最後に Cmd+V で全文をペースト
- **アイコン抽出** (v0.10.5) — `/Applications` 配下の全アプリのアイコンを起動時にバックグラウンドキャッシュ。ピッカーやパネルへの DnD で「選んだ瞬間」アイコンが出る
- **アプリピッカーの Launchpad 風グリッド** (v0.10.6) — 設定画面からのアプリ追加が `LazyVGrid` の 96px セルで一覧でき、ダブルクリック / Enter で即追加
- **プリセット機構** — グループとボタン、メニューバーから即切替
- **プリセットメモ** — プリセットごとに使う前提条件（前面アプリ / OS 設定 / クリップボード上書き等）を残せる。パネル上部に折りたたみブロックで表示、時間を空けて使い直す時に「忘れていたけど使う前にこれを確認」を即座に思い出せる
- **キー記録ボタンと特殊キーピッカー** (v0.9.2) — key アクションボタンを編集するとき、Delete・矢印・F1〜F20 等の特殊キーをそのまま 1 回押すだけで登録可能。プルダウンからも選べる。旧版の「キー名を TextField に手入力」では Delete/矢印を物理的に入力できなかった問題を解消
...
最新リリース: **v0.15.0** (2026-05-07)。変更履歴は [CHANGELOG.md](CHANGELOG.md)、ロードマップは [SPEC.md §17](SPEC.md) を参照してください。公開リリースですが、作者は安定性を保証しません。

> **Pull Request について:** 現在1日あたり 1,000〜5,000 行のペースでコードを書き換えており、PR はレビュー前にコンフリクトする可能性が非常に高いです。メンテナンスモードに移行するまで PR はお控えください。Issue での報告や機能提案は歓迎します。

> **Intel (x86_64) ビルドについて:** リリースには Intel を含むユニバーサルバイナリを同梱していますが、作者の Intel Mac が OS トラブルで現在使用不能です。Intel ビルドは Apple Silicon 上でクロスコンパイルしており、実機テストができない状況が当面続きます。Intel 固有の問題を見つけた場合は Issue で報告してください。
