...
final class ControlHandlers {
    ...
    /// displayName (preset の人間表示名) / visible (NSWindow の現在状態) /
    /// window 形状 / minimizedToEdge を含む。
    @MainActor
    private func handlePanelList() -> HTTPResponse {
        ...
        let entries: [[String: Any]] = panels.map { p in
            ...
            return [
                ...
                "visible": isVisible,
                "minimizedToEdge": p.minimizedToEdge,
                "window": [
                    "x": p.window.x,
                    "y": p.window.y,
    ...
    private func handlePanelShow(_ req: HTTPRequest) -> HTTPResponse {
        ...
        // ミニアイコンが表示中なら orderOut してから親パネルを出す。
    ...
    private func handleButtonUpdate(_ req: HTTPRequest) -> HTTPResponse {
        ...
        // confirm / confirmDestructive: Bool? — nil = leave unchanged.
        let confirm: Bool?            = dict["confirm"] as? Bool
        let confirmDestructive: Bool? = dict["confirmDestructive"] as? Bool
        // confirmMessage: String?? — keys.contains で「未指定 = nil」、null は .some(nil) で clear。
        let confirmMessage: String??  = dict.keys.contains("confirmMessage")
            ? (dict["confirmMessage"] as? String) : nil
        // thumbnail: 同じく Optional<Optional<String>> セマンティクス。
        // 未指定 = 変更なし、null = クリア、文字列 = 設定。
        let thumbnail: String?? = dict.keys.contains("thumbnail")
            ? (dict["thumbnail"] as? String) : nil
    ...
    private func handleButtonPress(_ req: HTTPRequest) -> HTTPResponse {
        ...
        guard let preset = presetManager.currentPreset else {
            return HTTPResponse.internalError("no active preset")
        }
        let button = preset.groups
            .flatMap { $0.buttons }
            .first { $0.id == id }
        guard let button else {
            return HTTPResponse.json([
                "error": "button not found in active preset",
                "id":    id,
                "preset": preset.name,
            ], status: 404)
        }

        // Bring the panel to front before clicking. Even at .floating
        // window level, other apps (notification panels, screen-share
        // overlays, etc.) can occlude us; if they do, the synthesized
        // mouse click would land on whatever is actually topmost at the
        // button's screen point, not on our button. orderFront() is a
        // no-op when we're already on top, so this is cheap.
        panel?.orderFront(nil)

        // Run the click on a background queue so the HTTP handler returns
        // immediately. The click itself takes ~150ms (cursor move + press
        // + small settle); blocking the main HTTP loop for that long
        // would serialize tests and obscure timing bugs.
        Task.detached {
            // Small settle so orderFront() has reached the window server
            // before we ask AX for our position. Without it, the click can
            // race with the raise and land under whatever was on top.
