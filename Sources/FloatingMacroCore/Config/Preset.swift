...
public struct ButtonGroup: Codable, Equatable {
    ...
    /// ならないようにするための後方互換配慮。それ以外のフィールドは合成 encoder
    /// と同じ挙動を維持する。
...
public struct PanelConfig: Codable, Equatable {
    /// 永続識別子。一度割り当てたら不変。デフォルトコンストラクタは UUID を生成する。
    public let id: String
    /// 表示するプリセット名（`presets/<name>.json` の `name` フィールド）。
    public var presetName: String
    /// ウィンドウ位置・サイズ・透明度などの表示属性。
    public var window: WindowConfig
    /// true のとき画面の縁にドックされた最小化バー状態として描画する。
    /// Phase 3 P3-5 で実装。デフォルト false。
    public var minimizedToEdge: Bool
    /// false のときメニューバー操作で隠されている。起動時は visible のものだけ
    /// 復元される（Phase 3 P3-6）。デフォルト true。
    public var visible: Bool
    /// パネル内のスクロール位置 (NSScrollView の clipView.bounds.origin.y 相当)。
    /// アプリ再起動時に表示位置を復元するために永続化する。デフォルト 0 (一番上)。
    /// 値は AppKit の点座標 (top-left オフセット ではなく Cocoa の bottom-up
    /// flipped 座標系基準) で、`PanelScrollView` が flipped documentView を使う
    /// 前提で「ピクセル数だけスクロール ダウンしている」ことを表現する非負値。
    public var scrollY: Double

    public init(id: String = UUID().uuidString,
                presetName: String,
                window: WindowConfig = WindowConfig(),
                minimizedToEdge: Bool = false,
                visible: Bool = true,
                scrollY: Double = 0) {
        self.id              = id
        self.presetName      = presetName
        self.window          = window
        self.minimizedToEdge = minimizedToEdge
        self.visible         = visible
        self.scrollY         = scrollY
    }

    private enum CodingKeys: String, CodingKey {
        case id, presetName, window, minimizedToEdge, visible, scrollY
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.minimizedToEdge = try c.decodeIfPresent(Bool.self, forKey: .minimizedToEdge) ?? false
...
public struct AppConfig: Codable, Equatable {
    ...
    private enum CodingKeys: String, CodingKey {
        case version, activePreset, window, controlAPI, commandBlacklist, seedInstalled, presetOrder, panels
    }
