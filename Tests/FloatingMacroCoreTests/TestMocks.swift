...
final class MockClipboard: ClipboardProtocol {
    ...
    /// Each Operation is recorded so tests can assert the exact sequence
    /// (e.g. "save -> set -> restore" for TextActionExecutor).
    enum Op: Equatable {
        case save
    ...
    /// Buffer that getString returns. setString updates it so appendMode
    /// behaviour can be verified end-to-end inside tests.
