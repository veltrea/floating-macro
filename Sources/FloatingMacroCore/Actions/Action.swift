...
public enum Action: Codable, Equatable {
    ...
    /// Inject text by writing to the clipboard and pressing Cmd+V.
    /// When `appendMode` is true, the executor instead reads the current
    /// clipboard string and concatenates `content` to its end without
    /// pasting — used for prompt-builder workflows where multiple buttons
    /// accumulate fragments and the user manually pastes the assembled
    /// result. In appendMode the `restoreClipboard` flag is ignored
    /// because the appended state must persist.
    case text(content: String, pasteDelayMs: Int, restoreClipboard: Bool, appendMode: Bool)
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "text":
            ...
            let appendMode = try container.decodeIfPresent(Bool.self, forKey: .appendMode) ?? false
            self = .text(content: content, pasteDelayMs: pasteDelayMs, restoreClipboard: restoreClipboard, appendMode: appendMode)
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .text(let content, let pasteDelayMs, let restoreClipboard, let appendMode):
            ...
            // Only emit appendMode when set, to keep older preset files clean.
            if appendMode {
                try container.encode(appendMode, forKey: .appendMode)
            }
