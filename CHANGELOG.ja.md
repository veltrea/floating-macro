...
### バグ修正 — マクロ編集でアクションのパラメータが失われる問題

- ボタン編集画面で `text` / `terminal` ステップを編集して保存すると、UI に出ていないパラメータ（`pasteDelayMs`・`restoreClipboard`・`appendMode`・terminal の `app` 等）が既定値に戻ってしまっていました。Control API で細かく設定したマクロが、編集画面で開いて保存しただけで設定が消える状態でした。全フィールドを保持するよう修正し、変換ロジックをコアの `MacroStepDraft` に分離してユニットテスト可能にしました。
- 単体の `delay` ボタンが編集できず `text` ステップとして誤表示されていました。`delay` アクションのタブを追加して直接編集できるようにしました。
- 不正なマクロステップ（空のキー・数値でない待機時間など）が保存時に `compactMap` で黙って捨てられていました。行内に ⚠ 警告を出し、理由を表示して保存をブロックするようにしました（消えなくなりました）。
- `delay` に負の値を入れると実行時にクラッシュしていました（`UInt64` 変換の trap）。デコード時の範囲検証（1ms〜1時間）と実行時クランプの二段で防止しました。

### 改善 — マクロの停止機構とテスト実行

- 実行中のマクロを、そのボタンをもう一度押すことで停止できるようになりました（トグル）。連打しても並走してキー送出が交錯することがなくなりました。`MacroRunner` は各ステップの境界でキャンセルを確認し、`stopOnError` が false でも停止要求は常に優先されます。
- 保存せずに編集内容を試せる「3秒後にテスト実行」ボタンをアクション編集画面に追加しました。カウントダウンの間に対象アプリへフォーカスを移せます。実行中はボタンが停止操作に変わります。
- ステップの複製ボタン、よく使う待機時間のクイック選択、ステップ種別の日本語表示を追加しました。

### バグ修正 — フローティングパネルが macOS 15 のウィンドウタイリングに巻き込まれる

- フローティングパネルを画面端へドラッグすると macOS 15（Sequoia）のウィンドウタイリングが発動し、パネルが強制的に半面表示や最大化されてしまっていました。OS によるウィンドウ移動（`isMovable` / `isMovableByWindowBackground`）を無効化し、自前のドラッグ移動に置き換えることで、構造的にタイリングの対象から外しました。

### バグ修正 — ドックバーの向きと端への吸着

- 上端で畳んだドックバー（横書き）を左右の端へ動かしても横書きのまま固定され、畳み直しても直りませんでした。ドロップ位置から向きを判定し直し、最寄りの辺が変わればバーを新しい向き（縦書き／横書き）で作り直すようにしました。辺が食い違った古い設定データも読み込み時に自己修復します。
- 向きの変更後、バーがドロップ位置のまま端から浮いて残っていました。判定した辺へ吸着させ、辺に沿う方向の位置は維持するようにしました。

### 内部

- `preset_create` / `preset_delete` がユーザー側プリセット（`~/Documents/FloatingMacro`）を解決できず、API で作成したプリセットが削除できなくなる問題を修正しました。
- テストを 490 件に拡充しました（Action の Codable ラウンドトリップ、マクロステップ変換、delay 範囲検証、ブラックリスト連携、キャンセル動作）。全件パスしています。
