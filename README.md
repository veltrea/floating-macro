...
The control API binds to `127.0.0.1:17430` automatically when the app starts. All endpoints require a Bearer token (except discovery endpoints like `/manifest`). The token is auto-generated on first launch and stored in macOS Keychain — you don't have to set anything up.
...
Get the token with `security find-generic-password -s FloatingMacro -a ControlAPIToken -w`.
...
Latest release: **v0.5** (2026-04-29). See [CHANGELOG.md](CHANGELOG.md) for the change history and [SPEC.md §17](SPEC.md) for the roadmap. Public release, but the author makes no guarantees of stability. Pull requests and issue reports are welcome.
