...
enum BinaryIdentity {
    ...
    /// Detect rebuild via hash change and log it. Resetting/relaunching is
    /// NOT done automatically — both `tccutil reset` and explicit
    ...
    /// in `AccessibilityChecker.isTrusted` (returns false even when
    /// `AXIsProcessTrusted()` lies with stale TRUE).
    static func handleStartupCheck(bundleId: String) {
        ...
        // Hash changed — TCC entry is now stale and silently revoked. We
