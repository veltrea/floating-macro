                    ...
                    Group {
                        ...
                        labeled("アイコン") {
                            HStack(alignment: .top, spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 6) {
                                    ...
                                    Button("アプリから...") { showingAppIconPicker = true }
                                        .help("インストール済みアプリのアイコンから選ぶ")
                                    Button("クリア") { iconPath = "" }
                                        .disabled(iconPath.isEmpty)
                                    Text("画像をドロップするか枠をクリックすると、preset 配下にコピーして登録します。")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                        ...
                        labeled("サムネイル (card タイプのグループ用)") {
                            HStack(alignment: .top, spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 6) {
                                    Button("クリア") { thumbnailPath = "" }
                                        .disabled(thumbnailPath.isEmpty)
                                    Text("親グループの表示タイプが card のときに大きく表示されます。icon / wide では無視されます。")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                        .fixedSize(horizontal: false, vertical: true)
                                    Picker("表示モード", selection: $cardThumbnailMode) {
                                        Text("クロップ").tag(CardThumbnailMode.fill)
                                        Text("全体表示").tag(CardThumbnailMode.fit)
                                    }
                                    .pickerStyle(.segmented)
                                    .frame(maxWidth: 200)
                                    Text(cardThumbnailMode == .fill
                                        ? "画像を拡大して正方形を埋める。はみ出す部分は切り取り。"
                                        : "画像全体をアスペクト比を保って表示。余白あり。")
                                        .font(.caption)
                                        .foregroundColor(.secondary)
                                        .fixedSize(horizontal: false, vertical: true)
                                }
    ...
    fileprivate static func importImage(
        ...
        guard let presetName = presetManager.currentPreset?.name else {
            presetManager.showTransientError(
                "アクティブな preset が見つかりません。", clearAfter: 4)
            return nil
        }
        do {
            ...
            presetManager.showTransientError(
                "画像の取り込みに失敗: \(error.localizedDescription)",
                clearAfter: 4
            )
            return nil
        }
...
struct GroupEditor: View {
                    ...
                    labeled("ボタンの表示タイプ") {
                        VStack(alignment: .leading, spacing: 4) {
                            Picker("", selection: $displayType) {
                                Text("icon (小さなアイコン)").tag(GroupDisplayType.icon)
                                Text("wide (横長セル)").tag(GroupDisplayType.wide)
                                Text("card (大きなサムネイル)").tag(GroupDisplayType.card)
                            }
                            .labelsHidden()
                            .pickerStyle(.segmented)
    ...
    private func displayTypeHint(_ type: GroupDisplayType) -> String {
        switch type {
        case .icon: return "既存の小さなアイコン+ラベルを縦に並べる、コンパクトな表示。"
        case .wide: return "全幅の横長セル。長いラベルや、視認性を優先したいボタンに。"
        case .card: return "サムネイル画像 + タイトルを 2 列のグリッドに配置。プロンプトギャラリー向け。"
        }
    }
