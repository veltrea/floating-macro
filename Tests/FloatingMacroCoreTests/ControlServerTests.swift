...
final class ControlServerTests: XCTestCase {
    ...
    func testDefaultBindScopeIsLoopback() throws {
        ...
        defer { s.stop() }
        XCTAssertEqual(s.currentBindScope, .loopback)
    }
...
This guarantees only that it is not broken)
        ...
        let s = ControlServer(preferredPort: port,
                              maxPortProbes: 10,
                              bindScope: .anyInterface) { _ in
            HTTPResponse.json(["scope": "any"])
        }
        defer { s.stop() }
        switch s.start(timeout: 2.0) {
        case .success(let bound):
            XCTAssertEqual(s.currentBindScope, .anyInterface)
            let (status, data) = try fetch(port: bound, path: "/")
            XCTAssertEqual(status, 200)
            let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
            XCTAssertEqual(json?["scope"] as? String, "any")
        case .failure(let err):
            XCTFail("anyInterface server failed to bind: \(err)")
        }
    }
