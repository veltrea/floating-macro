...
public enum ToolCatalog {
    ...
    public static let tools: [ToolDefinition] = [
        ...
        .init(name: "panel_list",
              description: "List all floating panels. Each entry has id, presetName, displayName, visible flag, window geometry (x/y/width/height/opacity), and dockedEdge (null or left/right/top/bottom).",
        ...
        .init(name: "panel_create",
              ...
              inputSchema: object([
                  ...
                  "backgroundColor": stringSchema(description: "#RRGGBB hex for the panel background. Omit for system default."),
              ], required: ["presetName"])),
        ...
        .init(name: "panel_background_color",
              description: "Set the background color of the panel with the given id. Color is a #RRGGBB hex string. Pass null or omit color to reset to the system default. Persists to config.json.",
              method: "POST", path: "/panel/background-color",
              inputSchema: object([
                  "id":    stringSchema(description: "Panel id from panel_list."),
                  "color": stringSchema(description: "#RRGGBB hex string, or null to reset to system default."),
              ], required: ["id"])),
        ...
        .init(name: "panel_dock",
              description: "Dock a panel to a screen edge as a thin bar. The panel is hidden and replaced by a small label bar at the edge. Click the bar to expand. Omit edge to auto-detect the nearest edge from the panel's current position.",
              method: "POST", path: "/panel/dock",
              inputSchema: object([
                  "id":   stringSchema(description: "Panel id from panel_list."),
                  "edge": stringSchema(description: "Screen edge: left, right, top, or bottom. Omit to auto-detect."),
              ], required: ["id"])),
        ...
        .init(name: "panel_reset_dock_position",
              description: "Reset a dock bar's custom position (set by dragging) back to auto-layout. Useful when a dock bar is dragged off-screen and becomes unreachable.",
        ...
        .init(name: "panel_gather_dock_bars",
              description: "Reset ALL dock bars to auto-layout positions. Rescue operation when bars are dragged off-screen.",
        ...
        .init(name: "preset_get",
              description: "Full JSON of an arbitrary preset by name (read-only; does not change the active preset). Use preset_list to discover names. Returns the same shape as preset_current.",
              method: "GET", path: "/preset/get",
              inputSchema: object([
                  "name": stringSchema(description: "Preset internal id (file name without .json) from preset_list."),
              ], required: ["name"])),
        ...
        // MARK: - Groups
        .init(name: "group_add",
              description: "Append a new button group to the active preset. Optional `displayType` controls how the group's buttons are rendered: `icon` (default, compact icon+label rows), `wide` (full-width row, big label), or `card` (large thumbnail + title — pair with each button's `thumbnail` field for a Midjourney-style prompt gallery). For card groups, `columns` controls the grid layout.",
              ...
              inputSchema: object([
                  ...
                  "displayType": stringSchema(description: "icon | wide | card | grid. Default icon."),
                  "columns":     stringSchema(description: "Card grid columns. \"auto\" (responsive, min 120pt cell) or 1/2/3 (fixed). Default auto. Only affects card displayType."),
                  "iconSize":    stringSchema(description: "Icon display size: small (16pt), medium (32pt, default), large (48pt), xlarge (64pt)."),
              ], required: ["id", "label"])),

        .init(name: "group_update",
              description: "Patch a group's label, icon, colors, tooltip, collapsed state, displayType, and/or columns. Pass `displayType` as one of `icon` / `wide` / `card` to switch the rendering style for all buttons in this group. `columns` controls the card grid: \"auto\" for responsive or 1/2/3 for fixed column count.",
              ...
              inputSchema: object([
                  ...
                  "displayType":     stringSchema(description: "icon | wide | card | grid."),
                  "columns":         stringSchema(description: "auto | 1 | 2 | 3. Card grid column count."),
                  "iconSize":        stringSchema(description: "small | medium | large | xlarge. Icon display size."),
              ], required: ["id"])),
    ...
    private static func buttonDefinitionSchema() -> [String: Any] {
        object([
            ...
            "cardThumbnailMode":  stringSchema(description: "fill | fit. Per-button. fill = crop to fill the square (default), fit = show the full image inside the square with letterbox."),
            "action":             actionSchema(),
    ...
    private static func buttonUpdateSchema() -> [String: Any] {
        object([
            ...
            "cardThumbnailMode":  stringSchema(description: "fill | fit. fill = crop to fill the square (default), fit = show the full image inside the square with letterbox."),
            "action":             actionSchema(),
        ], required: ["id"])
    }
