...
#     リッスンしていること
#   - Bearer トークンが macOS Keychain に保存されていること
#     (security find-generic-password -s FloatingMacro -a ControlAPIToken)
#
# Usage:
...
if [ -z "$TOKEN" ]; then
  echo "✗ Failed to fetch Bearer token from Keychain"
  echo "  Expected entry: service=FloatingMacro account=ControlAPIToken"
  exit 1
fi
