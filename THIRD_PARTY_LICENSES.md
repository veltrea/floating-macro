...
## WaterfallGrid (Swift Package)

A waterfall grid layout view for SwiftUI used by the floating panel's
card / grid display modes (gallery-style button layouts).

- Repository: <https://github.com/paololeonardi/WaterfallGrid>
- License: **MIT**

---

## libwebp (via libwebp-Xcode, Swift Package)

Used by the Web Panel feature's icon renderer to encode WebP images
(macOS's `ImageIO` decodes but does not encode WebP).

- Source: <https://chromium.googlesource.com/webm/libwebp> (upstream)
