        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                ...
                if model.lanExposed && model.bonjourReady {
                    Label("Bonjour 公開中", systemImage: "wifi")
            ...
            if model.lanExposed {
                ...
                HStack {
                    Text("ephemeral token: \(String(model.token.prefix(8)))…")
                        ...
                        .foregroundStyle(.secondary)
                    Spacer()
                    Button("再発行", action: onRotate)
                }

                Text("LAN 内のスマホ / タブレットの Safari で QR を読むと、上の URL が開きます。再発行すると過去の QR は無効になります。")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            } else {
                VStack(alignment: .leading, spacing: 8) {
                    Text("LAN 公開モードを ON にすると、同じ Wi-Fi にいるスマホ / タブレットからこのパネルを操作できます。")
                        .font(.callout)
                    Text("OS のファイアウォールが初回有効化時に「FloatingMacro が着信接続を受け付けるか」を聞いてくることがあります。許可してください。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
