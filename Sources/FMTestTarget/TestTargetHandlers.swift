...
struct TestTargetHandlers {
    ...
    let getSelection:   () -> (location: Int, length: Int)
    let quit:           () -> Void
    ...
    func dispatch(_ req: HTTPRequest) -> HTTPResponse {
        ...
        case (.GET,  "/selection"): return handleSelection()
    ...
    /// Expose NSTextView.selectedRange so tests can verify cursor moves
    /// (after arrow keys) and selection states (after Cmd+A).
    /// Returns: {location: Int, length: Int} — character offsets into the
    /// current text. location == cursor position when length == 0.
    private func handleSelection() -> HTTPResponse {
        let r = getSelection()
