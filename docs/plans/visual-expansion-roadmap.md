...
- [ ] **P3.6-1** `panel_move` `{id, x, y}` — 既存パネルを id 指定で移動
- [ ] **P3.6-2** `panel_resize` `{id, width, height}` — id 指定でリサイズ（min 120×80 クランプ）
- [ ] **P3.6-3** `panel_opacity` `{id, opacity}` — id 指定で透明度（0.25–1.0 クランプ）
- [ ] **P3.6-4** `panel_set_preset` `{id, presetName}` — id 指定でプリセット切替
- [ ] **P3.6-5** Tests: ToolCatalog 登録確認 + ControlHandlers ハンドラの単体検証
- [ ] **P3.6-6** AI_PROTOCOL.md / .ja.md にツール追記
