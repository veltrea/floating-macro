...
public enum PresetEditor {
    ...
    public static func addButton(
        ...
        afterButtonId: String? = nil,
        in preset: Preset
    ) throws -> Preset {
        return try updateGroup(groupId: groupId, in: preset) { g in
            if !g.buttons.contains(where: { $0.id == button.id }) {
                if let after = afterButtonId,
                   let idx = g.buttons.firstIndex(where: { $0.id == after }) {
                    g.buttons.insert(button, at: idx + 1)
                } else {
    ...
    public static func updateMemo(_ newMemo: String?, of preset: Preset) -> Preset {
        var updated = preset
        let trimmed = newMemo?.trimmingCharacters(in: .whitespacesAndNewlines)
        updated.memo = (trimmed?.isEmpty ?? true) ? nil : trimmed
        return updated
    }
}
...
public extension ButtonDefinition {
    ...
    mutating func patch(label: String? = nil,
        ...
        if let confirm = confirm { self.confirm = confirm }
        if let confirmMessage = confirmMessage { self.confirmMessage = confirmMessage }
        if let confirmDestructive = confirmDestructive { self.confirmDestructive = confirmDestructive }
        if let thumbnail = thumbnail { self.thumbnail = thumbnail }
        if let cardThumbnailMode = cardThumbnailMode { self.cardThumbnailMode = cardThumbnailMode }
...
public extension ButtonGroup {
    mutating func patch(label: String? = nil,
        ...
        if let displayType = displayType { self.displayType = displayType }
        if let columns = columns { self.columns = columns }
        if let iconSize = iconSize { self.iconSize = iconSize }
        if let showLabels = showLabels { self.showLabels = showLabels }
    }
}
