...
final class ToolCatalogTests: XCTestCase {
    ...
    func testPhase3PanelToolsAreRegistered() {
        ...
        let expectedPaths: [String: (String, String)] = [
            ...
            "panel_close":  ("POST", "/panel/close"),
            "panel_show":   ("POST", "/panel/show"),
            "panel_hide":   ("POST", "/panel/hide"),
        ]
        for (name, expected) in expectedPaths {
            guard let tool = ToolCatalog.find(name) else {
                XCTFail("missing panel tool: \(name)")
                continue
            }
            XCTAssertEqual(tool.method, expected.0, "method mismatch for \(name)")
            XCTAssertEqual(tool.path, expected.1, "path mismatch for \(name)")
        }
    }
    ...
    func testPhase3WindowToolsMarkedDeprecated() {
        ...
        for name in legacyNames {
            guard let tool = ToolCatalog.find(name) else {
                XCTFail("missing legacy window tool: \(name)")
                continue
            }
            XCTAssertTrue(tool.description.contains("DEPRECATED"),
                          "\(name) description should mention DEPRECATED, got: \(tool.description)")
        }
    }

    func testPhase3PanelCreateRequiresPresetName() {
        guard let tool = ToolCatalog.find("panel_create"),
              let required = tool.inputSchema["required"] as? [String] else {
            return XCTFail("panel_create schema malformed")
        }
        XCTAssertTrue(required.contains("presetName"),
                      "panel_create must require presetName")
    }

    // MARK: - Phase 3.6: per-id panel control tools

    func testPhase36PanelToolsAreRegistered() {
        let expected: [String: (String, String)] = [
            "panel_move":       ("POST", "/panel/move"),
            "panel_resize":     ("POST", "/panel/resize"),
            "panel_opacity":    ("POST", "/panel/opacity"),
            "panel_set_preset": ("POST", "/panel/set-preset"),
        ]
        for (name, exp) in expected {
            guard let tool = ToolCatalog.find(name) else {
                XCTFail("missing Phase 3.6 panel tool: \(name)")
                continue
            }
            XCTAssertEqual(tool.method, exp.0, "method mismatch for \(name)")
            XCTAssertEqual(tool.path,   exp.1, "path mismatch for \(name)")
        }
    }

    func testPhase36PanelToolsRequireId() {
        let names = ["panel_move", "panel_resize", "panel_opacity", "panel_set_preset"]
        for name in names {
            guard let tool = ToolCatalog.find(name),
                  let required = tool.inputSchema["required"] as? [String] else {
                XCTFail("\(name) schema malformed")
                continue
            }
            XCTAssertTrue(required.contains("id"),
                          "\(name) must require id (per-id panel control)")
        }
    }

    func testPhase36PanelMoveRequiresXY() {
        guard let tool = ToolCatalog.find("panel_move"),
              let required = tool.inputSchema["required"] as? [String] else {
            return XCTFail("panel_move schema malformed")
        }
        XCTAssertTrue(required.contains("x"), "panel_move must require x")
        XCTAssertTrue(required.contains("y"), "panel_move must require y")
    }

    func testPhase36PanelResizeRequiresWidthHeight() {
        guard let tool = ToolCatalog.find("panel_resize"),
              let required = tool.inputSchema["required"] as? [String] else {
            return XCTFail("panel_resize schema malformed")
        }
        XCTAssertTrue(required.contains("width"),  "panel_resize must require width")
        XCTAssertTrue(required.contains("height"), "panel_resize must require height")
    }

    func testPhase36PanelOpacityClampSchema() {
        guard let tool = ToolCatalog.find("panel_opacity"),
              let props = tool.inputSchema["properties"] as? [String: Any],
              let opacity = props["opacity"] as? [String: Any] else {
            return XCTFail("panel_opacity schema malformed")
        }
        XCTAssertEqual(opacity["minimum"] as? Double, 0.25,
                       "panel_opacity minimum must be 0.25")
        XCTAssertEqual(opacity["maximum"] as? Double, 1.0,
                       "panel_opacity maximum must be 1.0")
    }

    func testPhase36PanelSetPresetRequiresPresetName() {
        guard let tool = ToolCatalog.find("panel_set_preset"),
              let required = tool.inputSchema["required"] as? [String] else {
            return XCTFail("panel_set_preset schema malformed")
        }
        XCTAssertTrue(required.contains("presetName"),
                      "panel_set_preset must require presetName")
    }

    // MARK: - Phase 3.5: edge dock tools

    func testPhase35DockToolsAreRegistered() {
        let expected: [String: (String, String)] = [
            "panel_dock":   ("POST", "/panel/dock"),
            "panel_undock": ("POST", "/panel/undock"),
        ]
        for (name, exp) in expected {
            guard let tool = ToolCatalog.find(name) else {
                XCTFail("missing Phase 3.5 dock tool: \(name)")
                continue
            }
            XCTAssertEqual(tool.method, exp.0, "method mismatch for \(name)")
            XCTAssertEqual(tool.path,   exp.1, "path mismatch for \(name)")
    ...
    func testPhase35DockToolsRequireId() {
        for name in ["panel_dock", "panel_undock"] {
            guard let tool = ToolCatalog.find(name),
                  let required = tool.inputSchema["required"] as? [String] else {
                XCTFail("\(name) schema malformed")
                continue
            }
            XCTAssertTrue(required.contains("id"), "\(name) must require id")
        }
    }

    func testPhase35DockEdgeIsOptional() {
        guard let tool = ToolCatalog.find("panel_dock"),
              let required = tool.inputSchema["required"] as? [String] else {
            return XCTFail("panel_dock schema malformed")
        }
        XCTAssertFalse(required.contains("edge"), "edge should be optional")
    }
