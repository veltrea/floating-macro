...
### Panel (Phase 3 / v0.12 — preferred)
- `panel_list` — returns all panels as `{id, presetName, displayName, visible, window, dockedEdge}`
- `panel_create` `{presetName: String, x?, y?, width?, height?, opacity?}` — create a new panel; use the returned `id` for follow-up calls
- `panel_close` `{id}` — delete a panel (refused if it would leave zero panels)
- `panel_show` `{id}` — orderFront (also expands from dock / mini icon)
- `panel_hide` `{id}` — orderOut
- `panel_dock` `{id, edge?}` — dock a panel to a screen edge (left/right/top/bottom). Omit `edge` to auto-detect nearest edge
- `panel_undock` `{id}` — expand a docked panel back to normal floating state

### Panel — per-id control (Phase 3.6)
Move / resize / opacity / preset switching for any panel by id. Designed for users who can't drag windows easily and want to drive the layout via natural-language instructions ("move the right-hand panel to the bottom-left", "make the Claude Code panel cover the right half"). Pure-config layer is shared with the Settings UI, so changes persist to `config.json` and survive restarts.
- `panel_move` `{id, x, y}` — absolute screen coordinates (AppKit: origin = bottom-left of screen)
- `panel_resize` `{id, width, height}` — width ≥ 120, height ≥ 80 (clamped)
- `panel_opacity` `{id, opacity}` — clamped to `[0.25, 1.0]`
- `panel_set_preset` `{id, presetName}` — switch which preset that panel renders; `presetName` is the internal id from `preset_list`

### Window (deprecated — operates on the primary panel)
After v0.12 the app supports multiple floating panels; `window_*` is kept as a backward-compatible alias that targets `panels[0]`. New code should use `panel_*`.
...
- `preset_get` `{name}` — read-only fetch of any preset by name (not just the active one)
- `preset_switch` `{name}`
- `preset_reload`
- `preset_create` `{name?, displayName?, memo?}` — `memo` lets you record prerequisites (frontmost app, OS settings, side effects) at creation time
- `preset_rename` `{name, displayName?, memo?}` — `preset_update` in spirit; updates `displayName` and/or `memo`. Pass `memo: ""` to clear the memo. Tool name kept for compatibility.
- `preset_delete` `{name}`
- `preset_reorder` `{ids: [String]}` — replace the picker order. Names not on disk are dropped; missing names are appended in alphabetical order.
...
`displayType` (added in v0.11) controls how the group's buttons are rendered: `icon` (default, compact icon+label rows) / `wide` (full-width row, big label) / `card` (thumbnail + title gallery). When set to `card`, each button's `thumbnail` field is displayed prominently.
...
`button_update` accepts `thumbnail: null` (clear) or `thumbnail: "..."` (set). Only meaningful when the parent group's `displayType` is `card` (v0.11).
...
- `settings_set_key_combo` `{combo?, cmd?, shift?, option?, ctrl?, key?}` — configure the button to send a keyboard shortcut when clicked. Accepts a combo string (`"combo":"cmd+shift+v"`) or individual modifier flags. Switches the action type to `key` automatically. Special keys (`delete`, `forwarddelete`, `left/right/up/down`, `home/end`, `pageup/pagedown`, `return`, `tab`, `space`, `escape`, `f1`–`f20`) are supported (v0.9.2). ButtonEditor only
...
- `list_key_codes` `{}` (v0.9.2) — return the canonical catalog of key names usable in a combo string. Includes `modifiers` / `modifierAliases` / `specialKeys` (name + label) / `functionKeys` / `keyAliases` / `examples`. Use when you don't remember whether a special key is supported or when offering the user a picker
...
// Text append mode (v0.10 — prompt builder)
// When appendMode is true, the executor reads the current clipboard string,
// concatenates `content` to its end, writes back, and does NOT paste or
// restore. Used for prompt-building workflows (Midjourney style/pose/outfit
// fragments) where the user accumulates fragments across multiple button
// presses and pastes the final assembly manually with Cmd+V. The executor
// inserts no separator — include ", " etc. in content if you need one.
{
  "type": "text",
  "content": "anime style, ",
  "appendMode": true
...
Current version: `0.8`. Breaking changes bump the `version` in `/manifest`.
...
## 13. Web Panel — control from phone / tablet (v0.13)

### Overview

When LAN exposure mode is enabled, mobile devices on the same Wi-Fi can
access `/webpanel` to interact with the panel UI. Authentication uses an
**ephemeral LAN token** (invalidated on restart) rather than the permanent
Bearer token.
...
|---|---|---|
| GET | `/webpanel` | Main UI (HTML SSR) |
| GET | `/webpanel/assets/*` | CSS/JS/images |
| GET | `/webpanel/api/preset` | Current preset JSON |
| POST | `/webpanel/api/press` | Execute a button |

### Authentication

Web Panel requests carry the token as a URL parameter `?token=<ephemeral>`.
This is necessary because `<img>` tags cannot send Authorization headers.

```bash
# Retrieve the LAN token
curl -s -H "Authorization: Bearer $TOKEN" http://127.0.0.1:17430/lan-token

# Rotate the LAN token
curl -X POST -H "Authorization: Bearer $TOKEN" http://127.0.0.1:17430/lan-token/rotate
```

### Tool whitelist (`WebPanelToolWhitelist`)

Only safe tools are callable from Web Panel (`button_press`, `preset_list`,
`preset_current`, `preset_get`, etc.). Destructive or config-modifying tools
return 403.

### mDNS service type

`_floatingmacro._tcp.` — used for zero-config discovery by Bonjour-capable
clients.

---

## 14. See also — related protocols
