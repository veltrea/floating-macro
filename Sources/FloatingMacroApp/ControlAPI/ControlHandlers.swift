...
final class ControlHandlers {
    ...
    private func handlePresetCreate(_ req: HTTPRequest) -> HTTPResponse {
        ...
        // Both keys are optional. Missing `name` → auto-number; missing
        // `displayName` → fall back to the resolved name. This matches the
        // GUI flow where users only enter a display label.
