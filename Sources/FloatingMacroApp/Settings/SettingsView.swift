        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                Menu {
                    ...
                    Button("並べ替え…",          action: { showingPresetReorderSheet = true })
                    Divider()
                ...
                .frame(width: 28)
                .help("リネーム / 並べ替え / エクスポート / インポート")
            }

            // プリセットメモ
            HStack {
                Text("メモ").font(.caption).foregroundColor(.secondary)
                Spacer()
                if !memoText.isEmpty {
                    Text("\(memoText.count) 字")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                }
            }
            TextEditor(text: $memoText)
                ...
                .help("このプリセットを使う前提条件・注意点を書いておくと、パネル上部のメモアイコンから参照できます。")
            Text("使う前提（OS 設定 / 前面アプリ / クリップボード上書き等）を書いておくと、時間を空けて使い直す時にすぐ思い出せます。")
                .font(.caption2)
                .foregroundColor(.secondary)
            ...
            HStack {
                ...
                Button(action: { showingAppPickerSheet = true }) {
                    Label("アプリから追加…", systemImage: "app.badge")
                }
                .disabled(selectedGroupId == nil)
                .help("インストール済みアプリの一覧から起動ボタンを作成")
            }
        }
        .padding(8)
...
struct PresetReorderSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("プリセットの並べ替え")
                .font(.headline)
            Text("行をドラッグして順序を変更し「保存」を押してください。")
                .font(.caption)
                .foregroundColor(.secondary)
