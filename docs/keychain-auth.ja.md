...
> **重要 (v0.5+):** トークンの一次保存先は
> `~/Library/Application Support/FloatingMacro/control_api_token` (mode 0600)
> に変更されました。Keychain は `security` CLI 互換のためのミラーとして残っています。
> 新規環境では Keychain ダイアログは原則出ません。
> 取得は単に `cat ~/Library/Application\ Support/FloatingMacro/control_api_token`。
>
> 以下の説明は Keychain 経由での挙動を理解したい場合の参考情報です。

---

## 概要

FloatingMacro は起動時にランダムな Bearer トークンを生成し、
ファイル (`~/Library/Application Support/FloatingMacro/control_api_token`, mode 0600)
を一次ソースとして保存します。同時に macOS Keychain にもミラーされ、
従来の `security find-generic-password` CLI でも取得できます。
