...
public protocol ClipboardProtocol {
    ...
    /// Returns the current clipboard string, or nil if the clipboard contains
    /// no string-typed item. Used by `text` action's appendMode to read the
    /// existing clipboard before appending a new fragment.
